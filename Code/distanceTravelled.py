...
path = r'/Users/Sunny/RA/StopClassification/Data/raw/湘A-C1415'
...
for fileElement in allFiles:
    ...
    # overwite 'Distance traveled (km)' with distance calulated in meters by distance(lat1, long1, lat2, long2)
    for i in df.index:
        if i == 0:
            continue
    ...
    for i in df.index:
        if i == 0:
            continue
        df.at[i, 'Distance_geopy'] = distance_great_cirlce(df.at[i, 'Latitude'], df.at[i, 'Longitude'],
                                                           df.at[i - 1, 'Latitude'], df.at[i - 1, 'Longitude'])
